module ApplicationHelper
  ...
  def displaying_videos_thumbnail(source)
    if source.is_a?(Course)
      ...
      link_to(image_tag(video_thumbnail_image(video_to_display, {:medium => true}), :alt => "Image", :class => "img-responsive center-block video-thumbnail" ), course_topic_video_path(source.id, video_to_display.topic.id, video_to_display.id)) if video_to_display
    elsif source.is_a?(Topic)
      video_to_display = source.topic_first_video
      link_to(image_tag(video_thumbnail_image(video_to_display, {:medium => true}), :alt => "Image", :class => "img-responsive center-block video-thumbnail" ), course_topic_video_path(source.course_id, source.id, video_to_display.id)) if video_to_display
    end
  end
end
