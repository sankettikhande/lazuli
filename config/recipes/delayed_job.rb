...
namespace :delayed_job do
  ...
  task :start, :roles => lambda { roles } do
    find_servers_for_task(current_task).each do |server|
      run "cd #{current_path};#{rails_env} script/delayed_job #{args_per_host server.host} start", :hosts => server.host
    end
  end
  ...
  task :restart, :roles => lambda { roles } do
    find_servers_for_task(current_task).each do |server|
      run "cd #{current_path};#{rails_env} script/delayed_job #{args_per_host server.host} restart", :hosts => server.host
    end
  end
end